...
**Dev / build (Astro)** — the home is now an Astro build:
```
npm install
npm run dev         # dev server with hot-reload
npm run build       # generates static dist/
npm run check:dist  # verifies the output (node --test tests/dist/)
```
Every other page is still plain static HTML served from `public/`.
...
Site content exists in 3 languages as **fully duplicated HTML files**, not templates: Spanish, English, and Portuguese, each page mirrored as `configurador.html`, `guias.html`, `sobre.html`, `contacto.html`, `privacidad.html`, `niveles/*.html`, `guias/*.html`. All three trees share the same `css/style.css` and `js/*.js` via relative `../` paths. The active language is read from `<html lang="es|en|pt">`; `rutaLocalizada()` in `js/main.js` rewrites a Spanish data path (`data/foo.json`) to its localized sibling (`data/foo.en.json`) so every JS module that fetches data automatically gets the right language without extra logic. **Editing content in one language does not propagate** — a change has to be applied by hand to the matching file in the other two trees.

**What is actually deployed:** the non-home pages plus all `css/`, `js/`, `data/`, and `img/` assets live under `public/` (Spanish at `public/`, English under `public/en/`, Portuguese under `public/pt/`) and are copied verbatim into `dist/` by the Astro build. The root-level `en/`, `pt/`, `css/`, `js/`, `data/`, `niveles/`, `guias/`, and root `*.html` copies are **legacy duplicates that are NOT deployed** — editing them ships nothing; edit the `public/` copies.

The **home** (`/`, `/en/`, `/pt/`) no longer uses this duplicated-HTML pattern at all — it is generated by Astro from `src/pages/**/index.astro` + `src/i18n/*.json`. Every other page still does.
